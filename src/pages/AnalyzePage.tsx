...
function runMockAnalysis(input: AnalysisInput) {
  ...
  for (let i = 0; i < topicCount; i++) {
    ...
    const topicTitle = topWords.length > 0
    ...
    topics.push({
      id: `topic_${i + 1}`,
      title: topicTitle,
      summary: chunk.slice(0, 2).join(' ').substring(0, 200) + '...',
      keywords: topWords,
  ...
  const insights = sentences
    .filter((s) => /\d/.test(s) || /important|critical|key|significant|growth|increase|strategy/i.test(s))
    .slice(0, 6)
    .map((s, idx) => ({
      id: `ins_${idx + 1}`,
      text: s.substring(0, 180),
      type: /\d/.test(s) ? 'data-point' : 'action-insight',
    }));

  if (insights.length === 0 && sentences.length > 0) {
    insights.push({
      id: 'ins_1',
      text: sentences[0].substring(0, 180),
      type: 'key-point',
    });
  }
  ...
  const personaMatches = activePersonas.map((p) => {
    ...
    return {
      ...
      question: `How does this relate to ${meta.goal.substring(0, 60)}?`,
      tone: meta.tone,
    };
  ...
  const depthAnalysis = topics.map((t) => ({
    ...
    recommendation: t.sentenceCount <= 2
      ? 'Needs additional research and expert input'
      : 'Sufficient depth for content generation',
  }));
    ...
    for (const persona of matchedPersonas) {
      for (const format of typeConfig.formats.slice(0, 2)) {
        ...
        opportunities.push({
          ...
          personaRelevanceScore: persona.relevanceScore,
          personaPainPoint: persona.painPoint || null,
          personaQuestion: persona.question || null,
          contentAngle: `Address ${topic.title.toLowerCase()} from ${persona.personaName} perspective`,
          recommendedFormat: format,
          recommendationReason: `${format} format is ideal for ${typeConfig.label} content targeting ${persona.personaName}`,
          unsuitableFormatNotes: '',
          priority: persona.relevanceScore > 70 ? 'high' : 'standard',
          timeliness: sourceType === 'trending_topic' ? 'urgent' : 'standard',
          suggestedCTA: `Learn more about ${topic.keywords[0] || 'this topic'}`,
          expertAttribution: sourceOwner || 'Content Team',
          sourceContext: topic.summary.substring(0, 120),
          status: 'open',
          createdAt: Date.now(),
          analysisId: null, // will be set after analysis is stored
        });
      }
    }
  }
  ...
  /* --- Knowledge context --- */
  const usedFiles = kb.filter((f) => f.active).map((f) => ({
    fileId: f.id,
    fileName: f.fileName,
    category: f.category,
    relevance: f.category === 'persona' ? 'primary' : 'supporting',
  }));

  /* --- Source references --- */
  const sourceReferences = sourceUrl
    ? [{ url: sourceUrl, title: sourceTitle || 'Source Link', verified: false, note: 'URL provided - verification pending (integration not connected)' }]
    : [];

  /* --- Quality check --- */
  const qualityCheck = {
    groundingScore: insights.length > 2 ? 85 : 60,
    complianceFlag: kb.some((f) => f.active && f.category === 'compliance') ? 'checked' : 'skipped',
    brandAligned: kb.some((f) => f.active && f.category === 'brand'),
    warnings: [] as string[],
  };
  ...
  const sourceSummary = {
    ...
    owner: sourceOwner || 'Unknown',
    wordCount: sourceContent.split(/\s+/).length,
    sentenceCount: sentences.length,
    marketingNotes: marketingNotes || 'None provided',
  };
  ...
  return {
    ...
    depthAnalysis,
    opportunities,
    knowledgeContext: { filesUsed: usedFiles, totalFiles: kb.length, activeFiles: kb.filter((f) => f.active).length },
    sourceReferences,
    qualityCheck,
    analysisWarnings,
  };
...
export default function AnalyzePage() {
  ...
  const handleRunAnalysis = useCallback(() => {
    ...
    timerRef.current = setInterval(() => {
      step++;
      if (step < ACTIVITY_LABELS.length) {
        ...
        const analysisResult = runMockAnalysis({
          ...
          sourceTitle,
          sourceOwner,
          sourceContent: inputMode === 'url' ? `[Content from URL: ${sourceUrl}] ${sourceContent}` : sourceContent,
          sourceUrl,
          marketingNotes,
          kb,
        });
        ...
        const analysisItem: AnalysisItem = {
          ...
          sourceOwner: sourceOwner || 'Unknown',
          sourceUrl,
          ...analysisResult,
        };

        /* Tag opportunities with analysis id */
        const taggedOpps: OpportunityItem[] = analysisResult.opportunities.map((o: any) => ({
          ...o,
          analysisId,
        }));
        ...
        setActivityStep(ACTIVITY_LABELS.length); // all done
        setIsRunning(false);
      }
    }, 380);
  ...
  return (
    <div>
      ...
      <p className="page-desc">
        Paste your source content and let the intelligence engine extract topics, match personas, and identify content opportunities.
      </p>
      ...
      <div style={{ marginBottom: '1.2rem' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
          {SOURCE_TYPES.map((st) => (
            <span
              ...
              style={{
                cursor: 'pointer',
                background: sourceType === st.id ? 'var(--accent-primary)' : undefined,
                color: sourceType === st.id ? '#fff' : undefined,
                borderColor: sourceType === st.id ? 'var(--accent-primary)' : undefined,
              }}
            >
              {st.label}
            </span>
        ...
        <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginTop: '0.35rem' }}>
          Available formats: {typeConfig.formats.join(', ')}
        </div>
      </div>
      ...
      <div className="grid grid-2" style={{ gap: '1.5rem', alignItems: 'start' }}>
        ...
        <div>
          ...
          <div className="underline-tabs" style={{ marginBottom: '1rem' }}>
            <button
              className={`u-tab${inputMode === 'text' ? ' active' : ''}`}
              onClick={() => setInputMode('text')}
            >
              Paste Text / Transcript
            </button>
            <button
              className={`u-tab${inputMode === 'url' ? ' active' : ''}`}
          ...
          <div className="glass-card-static" style={{ padding: '1.2rem' }}>
            ...
            {/* Source URL (visible in url mode or always as optional) */}
            {inputMode === 'url' && (
              <div className="field" style={{ marginBottom: '0.8rem' }}>
                ...
                <div style={{
                  ...
                  URL fetch integration is not connected. Paste the article text below as well.
                </div>
              </div>
            )}
            ...
            <div className="field" style={{ marginBottom: '0.8rem' }}>
              <label className="field-label">
                {inputMode === 'text' ? 'Source Content / Transcript' : 'Paste Article Text (fallback)'}
              </label>
              <textarea
                className="glass-textarea"
                rows={8}
                placeholder={inputMode === 'text'
                  ? 'Paste the full transcript, article text, or raw content here...'
                  : 'Paste the article text here since URL fetch is not yet connected...'
                }
                value={sourceContent}
            ...
            <div className="field" style={{ marginBottom: '1rem' }}>
              ...
              <textarea
                className="glass-textarea"
                rows={3}
                placeholder="Any specific goals, campaigns, or context for this content..."
            ...
            {activityStep < 0 ? (
              <div className="empty-state" style={{ padding: '2rem 1rem' }}>
                <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                  Click "Run Analysis" to start the intelligence engine.
                </p>
              </div>
            ) : (
      ...
      {result && (
        <div style={{ marginTop: '2rem' }}>
          ...
          <div className="grid grid-4" style={{ gap: '0.8rem', marginBottom: '1.5rem' }}>
            <StatCard label="Topics" value={result.topics.length} color="var(--accent-primary)" />
            <StatCard label="Insights" value={result.insights.length} color="var(--status-info)" />
            <StatCard label="Opportunities" value={result.opportunities.length} color="var(--status-success)" />
            <StatCard
              label="Content Value"
              value={result.opportunities.length > 8 ? 'High' : result.opportunities.length > 4 ? 'Medium' : 'Low'}
              color="var(--status-warning)"
            />
          </div>
          ...
          {result.analysisWarnings.length > 0 && (
            <div className="glass-card-static" style={{ padding: '0.8rem 1rem', marginBottom: '1rem', borderLeft: '3px solid var(--status-warning)' }}>
              <div style={{ fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--status-warning)', marginBottom: '0.4rem' }}>
                Warnings
              </div>
              {result.analysisWarnings.map((w: string, i: number) => (
          ...
          {result.sourceReferences.length > 0 && (
            <div className="glass-card-static" style={{ padding: '1rem 1.2rem', marginBottom: '1rem' }}>
              ...
              {result.sourceReferences.map((ref: any, i: number) => (
                <div key={i} style={{ marginBottom: '0.4rem' }}>
                  <KV k="URL" v={ref.url} />
                  <KV k="Status" v={ref.verified ? 'Verified' : 'Unverified'} />
                  {ref.note && <KV k="Note" v={ref.note} />}
                </div>
              ))}
            </div>
          )}
          ...
          <div className="glass-card-static" style={{ padding: '1rem 1.2rem', marginBottom: '1rem' }}>
            ...
            <div className="grid grid-2" style={{ gap: '0.8rem' }}>
              {result.topics.map((topic: any, i: number) => {
                ...
                const depthColor = depth?.depth === 'Deep'
                  ? 'var(--status-success)'
                ...
                return (
                  <div key={i} className="glass-card-static" style={{ padding: '0.8rem', position: 'relative', overflow: 'hidden' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.3rem', flexWrap: 'wrap', marginBottom: '0.3rem' }}>
                      {topic.keywords.map((kw: string, ki: number) => (
                        <span key={ki} className="badge" style={{ fontSize: '0.65rem' }}>{kw}</span>
                      ))}
                    </div>
                    {depth && (
                      <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>
                        Depth: <span style={{ color: depthColor, fontWeight: 600 }}>{depth.depth}</span>
                        {' '} &middot; Coverage: {depth.coverage}%
                      </div>
                    )}
                  </div>
                );
              ...
              {result.insights.map((ins: any, i: number) => (
                <div key={i} style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.5rem', alignItems: 'flex-start' }}>
                  <span className="badge" style={{
                    fontSize: '0.6rem',
                    flexShrink: 0,
                    background: ins.type === 'data-point' ? 'var(--status-info)' : 'var(--accent-primary)',
                    color: '#fff',
          ...
          <div className="glass-card-static" style={{ padding: '1rem 1.2rem', marginBottom: '1rem' }}>
            <div style={{ fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--text-muted)', marginBottom: '0.6rem' }}>
              Knowledge Used ({result.knowledgeContext.activeFiles} / {result.knowledgeContext.totalFiles} files active)
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.35rem' }}>
              {result.knowledgeContext.filesUsed.map((f: any, i: number) => (
                <span key={i} className="badge" style={{
                  fontSize: '0.68rem',
                  background: f.relevance === 'primary' ? 'var(--accent-primary-soft)' : undefined,
                }}>
                  {f.fileName}
                </span>
              ))}
          ...
          {/* Content Opportunities link */}
          <div className="glass-card-static" style={{ padding: '1rem 1.2rem', textAlign: 'center' }}>
            <div style={{ fontSize: '0.85rem', marginBottom: '0.5rem' }}>
              <strong>{result.opportunities.length}</strong> content opportunities identified
            </div>
            <button
              className="btn btn-primary btn-sm"
              onClick={() => setActiveTab('opportunities')}
            >
              View in Opportunities Tab
            </button>
          </div>
        </div>
      )}
