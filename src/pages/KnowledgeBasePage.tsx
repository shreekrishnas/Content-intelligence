...
const KB_CATEGORIES = [
  ...
  { id: 'brand', label: 'Brand Tone of Voice', desc: 'Right Horizons writing guidelines, approved messaging' },
  { id: 'personaTone', label: 'Persona-Specific Tone', desc: 'Tone documents for individual approved personas' },
  { id: 'expert', label: 'Expert Voice Files', desc: 'Voice of Anil / Rachana / People — transcripts and writing samples' },
          ...
          {readiness.map((r) => (
            <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{
                ...
                background: r.ready ? '#10B981' : '#F59E0B',
              }} />
            ...
            return (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 180 }}>
                  ...
                  <p style={{ fontSize: 12, opacity: 0.6, marginTop: 2 }}>{file.description}</p>
                </div>
        ...
        <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) setShowUpload(false); }}>
          <div className="glass-modal">
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16 }}>
              ...
              <button className="btn btn-primary" onClick={handleUpload}>Add File</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) setPreviewFile(null); }}>
          <div className="glass-modal">
            ...
            <div style={{ fontSize: 13, marginBottom: 8 }}>
              <strong>Category:</strong> {categoryLabel(previewFile.category)}
            </div>
            <div style={{ fontSize: 13, marginBottom: 8 }}>
              <strong>Priority:</strong> {previewFile.priority}
            </div>
            <div style={{ fontSize: 13, marginBottom: 8 }}>
              <strong>Version:</strong> {previewFile.version}
            </div>
            <div style={{ fontSize: 13, marginBottom: 8 }}>
              <strong>Active:</strong> {previewFile.active ? 'Yes' : 'No'}
            </div>
            <div style={{ fontSize: 13, marginBottom: 8 }}>
              <strong>Description:</strong> {previewFile.description}
            </div>
            <div style={{ fontSize: 13, marginBottom: 8 }}>
              <strong>Sample file:</strong> {previewFile.sample ? 'Yes' : 'No'}
            </div>
            <div style={{ fontSize: 13, marginBottom: 8 }}>
              <strong>Uploaded:</strong> {new Date(previewFile.uploadedAt).toLocaleDateString()}
            </div>
            ...
            {previewFile.personaMeta && (
              <>
                ...
                <div style={{ fontSize: 13, marginBottom: 6 }}>
                  <strong>Name:</strong> {previewFile.personaMeta.name}
                </div>
                <div style={{ fontSize: 13, marginBottom: 6 }}>
                  <strong>Pain Point:</strong> {previewFile.personaMeta.painPoint}
                </div>
                <div style={{ fontSize: 13, marginBottom: 6 }}>
                  <strong>Goal:</strong> {previewFile.personaMeta.goal}
                </div>
                <div style={{ fontSize: 13, marginBottom: 6 }}>
                  <strong>Tone:</strong> {previewFile.personaMeta.tone}
                </div>
              </>
            )}
