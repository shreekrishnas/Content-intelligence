// ============================================================
// Shared core for the AI Trend Supervisor feature.
// (Files/folders under api/ prefixed with "_" are NOT deployed as
//  routes — this is a shared helper module, not an endpoint.)
// ============================================================
...
export async function callLLM(systemPrompt: string, userPrompt: string, options: { maxTokens?: number; temperature?: number } = {}): Promise<string> {
...
export function buildSupervisorPrompt(body: { domain_profile?: DomainProfile; signals?: TrendSignal[]; account_label?: string }): string {
...
export async function supervise(body: { domain_profile?: DomainProfile; signals?: TrendSignal[]; account_label?: string }): Promise<{ topics: any[]; summary: any; analysis_date?: string }> {
...
// Collect live signals via Tavily. Returns [] if TAVILY_API_KEY is not set or
// all requests fail (caller can then fall back to suggest mode).
export async function collectTavilySignals(profile: DomainProfile): Promise<TrendSignal[]> {
...
// Ask the LLM to propose CANDIDATE topics worth investigating (clearly framed
// as hypotheses, not confirmed live trends). Used when no live data source is
// configured, so the supervisor still has something to evaluate.
export async function suggestCandidateSignals(profile: DomainProfile, accountLabel?: string): Promise<TrendSignal[]> {
...
// Map a supervisor topic to a DB-insertable trend_records row shape.
